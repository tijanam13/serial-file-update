...
void procesuirajSlucaj4(void) {
	char datum[7];
	char oldMatPutanja[100], sumTranPutanja[100], rptPutanja[100], errProPutanja[100];
	char demoMatPutanja[100], demoTranPutanja[100];
	char rptIme[64], errIme[64];

	formirajDatum(datum);
	sprintf(rptIme, "prom_%s.rpt", datum);
	sprintf(errIme, "err_pro_%s.rpt", datum);
	...
	sprintf(errProPutanja, ".\\ERR\\%s", errIme);

	sprintf(demoMatPutanja, ".\\DEMO\\maticna.dat");
	sprintf(demoTranPutanja, ".\\DEMO\\SLUC_4\\transakciona.dat");
	...
	if (!dat_ucitaj_transakcije(TRAN_DAT, &vt)) {
		printf("Greska: Transakciona datoteka za slucaj 4 je prazna!\n");
		vt_free(&vt); return;
	}
	...
	FILE* fErr = fopen(errProPutanja, "w");
	...
	while (imaMat || s_idx < vtSum.size) {
		if (imaMat && s_idx < vtSum.size && mp.Id == vtSum.items[s_idx].Id) {
			...
			fprintf(fErr, "| %-2u | %8s | %12s |  %c  | %8u | %-30s |\n",
				st.Id, "X", "X", (st.Promena == ULAZ ? '+' : '-'), st.Kolicina, "Nepostojeci proizvod.");
	...
	printf("\n[Pritisnite ENTER za sledeci izvestaj: Izvestaj o greskama (Nepostojeci proizvod)]");
	getchar(); 

	system("cls");
	prikaziSadrzajIzvestaja(errProPutanja);

	printf("\nINFO: Slucaj 4 (Nepostojeci proizvod) uspesno obradjen.\n");
}